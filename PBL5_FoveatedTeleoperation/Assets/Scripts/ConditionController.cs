...
public class ConditionController : MonoBehaviour
{
    ...
    public void SetCondition(Condition c)
    {
        ...
        RobotClient rc = FindObjectOfType<RobotClient>();
    ...
    private void ConnectionLoop(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            // Safe lookup on main thread proxy or just checking regularly
            // Since FindObjectOfType can only be called from main thread, we will just manage our connection
            // unless we successfully route. If we fail to send or if there is no connection, we will connect.
            // Wait, we can check if RobotClient is in the scene inside Update/Start on the main thread,
            // but in the background thread we don't want to call FindObjectOfType.
            // So we can set a flag on main thread or just let it try connecting.
            // Actually, let's keep it simple: if RobotClient exists, we don't connect.
            // How does the background thread know if RobotClient exists?
            // We can check it on the main thread in Start() or Update() and set a boolean field `hasRobotClient`.
            ...
            if (!isConnected)
            {
                ...
                {
                    Debug.LogWarning($"[ConditionController] Connection failed: {ex.Message}");
                    isConnected = false;
    ...
    private void AwakeHelper()
    {
        ...
        hasRobotClient = FindObjectOfType<RobotClient>() != null;
    }
